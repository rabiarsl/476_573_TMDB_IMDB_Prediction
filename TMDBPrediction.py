...
def add_extra_features(df):
    ...
    df['weightedRating'] = (df['rating'] * df['totalVotes'] + 6.6 * 1000) / (df['totalVotes'] + 1000)
...
submission = pd.read_csv('sample_submission.csv')
submission['revenue'] = pred_cat
submission.to_csv("submission_cat.csv", index=False)

submission = pd.read_csv('sample_submission.csv')
submission['revenue'] = pred_xgb*0.45 + pred_cat*0.3 + pred_lgb*0.25
submission.to_csv("submission_xgb_lgb_cat.csv", index=False)